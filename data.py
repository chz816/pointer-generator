...
def example_generator(input_file, single_pass):
    ...
    Args:
      input_file:
        Path to tf.Example data files. Can include wildcards, e.g. if you have several training data chunk files train_001.bin, train_002.bin, etc, then pass input_file=train_* to access them all.
      single_pass:
    ...
    """
    while True:
        filelist = glob.glob(input_file)  # get the list of datafiles
        assert filelist, ('Error: Empty filelist at %s' % input_file)  # check filelist isn't empty
        if single_pass:
