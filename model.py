...
class SummarizationModel(object):
    ...
    def _add_placeholders(self):
        ...
        if self._hps.mode.value not in ['decode']:
            # decoder part
            ...
            self._dec_padding_mask = tf.placeholder(tf.float32, [hps.batch_size.value, hps.max_dec_steps],
                                                    name='dec_padding_mask')
            self._target_batch = tf.placeholder(tf.int32, [hps.batch_size.value, hps.max_dec_steps],
                                                name='target_batch')
            self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size.value, hps.max_dec_steps],
                                             name='dec_batch')
    ...
    def _add_decoder(self, inputs):
        ...
        prev_coverage = self.prev_coverage if hps.mode.value in ["decode"] and hps.coverage else None
        ...
        outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state,
                    ...
                    hps.mode.value == "decode"), pointer_gen=hps.pointer_gen.value, use_coverage=hps.coverage.value,
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            # Some initializers
            self.rand_unif_init = tf.random_uniform_initializer(-hps.rand_unif_init_mag.value,
                                                                hps.rand_unif_init_mag.value, seed=123)
            ...
            with tf.variable_scope('output_projection'):
                w = tf.get_variable('w', [hps.hidden_dim.value, vsize], dtype=tf.float32,
                                    initializer=self.trunc_norm_init)
                w_t = tf.transpose(w)
    ...
    def _add_train_op(self):
        ...
        optimizer = tf.train.AdagradOptimizer(self._hps.learning_rate.value, initial_accumulator_value=self._hps.adagrad_init_acc.value)

        with tf.device("/gpu:0"):
            self._train_op = optimizer.apply_gradients(zip(grads, tvars), global_step=self.global_step, name='train_step')

        #self._train_op = optimization.create_optimizer(total_loss, self._hps.learning_rate.value, self._hps.num_train_steps.value, self._hps.num_warmup_steps.value, self._hps.use_tpu.value)
