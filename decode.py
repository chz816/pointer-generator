    ...
    def __init__(self, model, batcher, vocab):
        ...
        if not FLAGS.generate:
            ...
            if FLAGS.single_pass and FLAGS.mode == 'decode':
                ...
                self._decode_dir = os.path.join(FLAGS.output_dir, get_decode_dir_name(ckpt_name))
                if os.path.exists(self._decode_dir):
                    raise Exception("single_pass decode directory %s should not already exist" % self._decode_dir)

            else:
                self._decode_dir = os.path.join(FLAGS.output_dir, "decode")
            ...
            self._result_dir = os.path.join(FLAGS.output_dir, "test-result")
    ...
    def decode(self):
        ...
        # when generate=True
            ...
            while True:
                ...
                # Write to file
                result_file = os.path.join(self._result_dir, "%06d_summary.txt" % counter)
...
def get_decode_dir_name(ckpt_name):
    ...
    if "train" in FLAGS.input_file:
        dataset = "train"
    elif "val" in FLAGS.input_file:
        dataset = "val"
    elif "test" in FLAGS.input_file:
        dataset = "test"
    else:
        raise ValueError("FLAGS.input_file %s should contain one of train, val or test" % (FLAGS.input_file))
