...
tf.app.flags.DEFINE_string('output_dir', '', 'Root directory for all logging.')

# num of training steps
tf.app.flags.DEFINE_integer('num_train_steps', 100000, 'Number of training steps')

# learning rate
tf.app.flags.DEFINE_float('learning_rate', 0.15, 'learning rate')

# max source text tokens
tf.app.flags.DEFINE_integer('max_enc_steps', 400, 'max timesteps of encoder (max source text tokens)')

# how often to save the model checkpoint
tf.app.flags.DEFINE_integer("save_checkpoints_steps", 1000, "How often to save the model checkpoint.")

tf.app.flags.DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.")
...
# for generating the text only
tf.app.flags.DEFINE_boolean('generate', False, 'Flag variable: generate only')
...
def restore_best_model():
    ...
    new_fname = os.path.join(FLAGS.output_dir, "train", new_model_name)
...
def setup_training(model, batcher):
    ...
    sv = tf.train.Supervisor(logdir=train_dir,
                             ...
                             saver=saver,
                             summary_op=None,
                             save_summaries_secs=30,  # save summaries for tensorboard every 30 secs
                             save_model_secs=30,  # checkpoint every 30 secs
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
    ...
    with sess_context_manager as sess:
        ...
        current_step = 0
        while current_step <= FLAGS.num_train_steps:  # repeats until interrupted
...
def run_eval(model, batcher, vocab):
    ...
    model.build_graph()  # build the graph
    # we will keep 2 best checkpoints at a time
    saver = tf.train.Saver(max_to_keep=2)
    sess = tf.Session(config=util.get_config())
    eval_dir = os.path.join(FLAGS.output_dir, "eval")  # make a subdir of the root dir for eval data
    bestmodel_save_path = os.path.join(eval_dir, 'bestmodel')  # this is where checkpoints of best models are saved
    summary_writer = tf.summary.FileWriter(eval_dir)
    running_avg_loss = 0  # the eval job keeps a smoother, running average loss to tell it when to implement early stopping
    best_loss = None  # will hold the best loss achieved so far
    #ckpt_dir = FLAGS.output_dir + "train/"
...
def main(_):
    ...
    hparam_list = ['init_checkpoint', 'mode', 'learning_rate', 'adagrad_init_acc', 'rand_unif_init_mag',
                   'trunc_norm_init_std', 'max_grad_norm',
                   'hidden_dim', 'emb_dim', 'batch_size', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt',
                   'pointer_gen', 'num_train_steps', 'use_tpu', 'num_train_steps', 'num_warmup_steps']
    hps_dict = {}
    ...
    batcher = Batcher(FLAGS.input_file, vocab, hps, single_pass=FLAGS.single_pass)
